...
int estado = 0;
bool play;
bool reversa; //Sentido horario-antihorario
...
//Maquina de estados para recorrido normal
void recorrer_pista_antihorario(void) {
	if (estado == 0) {

		rotKit_y != 270 ? rotKit_y = 270 : NULL;
		if (movKit_x > -13) {
			movKit_x -= 1.0f;
		}
		else {
	...
	if (estado == 3) {
		...
		rotKit_y != 180 ? rotKit_y = 180 : NULL;
		if (movKit_z > 50) {
			movKit_z -= 1.0f;
		}
		else {
...
//Maquina de estados para recorrido en reversa
void recorrer_pista_horario(void) {
	if (estado == 0) {
...
void animate(void)
{
	...
	if (play) {
		//Revisa el recorrido que debe hacer
		reversa ? recorrer_pista_horario() : recorrer_pista_antihorario();
